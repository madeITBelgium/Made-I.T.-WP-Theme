...
export default function save( props ) {
    ...
    const outerClassName = uniqClassString(
        classnames(
            stripManagedTokens( className || '' ),
            stripManagedTokens( wrapperClassName || '' )
        )
    );
    ...
    const widthRounded = Number.isFinite( width )
        ? Math.round( width )
        : undefined;
    ...
    if ( hasBackground && ! hasInnerWrapper ) {
        ...
        if ( rawWrapperClassName ) {
            ...
            const wrapperAlreadyHasBgClass =
                !! backgroundColorClass &&
                tokenizeClassString( rawWrapperClassName ).includes( backgroundColorClass );

            const legacyBlockProps = {
                className:
                    upsertVerticalAlignmentClass( rawWrapperClassName ) +
                    ( backgroundColorClass && ! wrapperAlreadyHasBgClass
                        ? ` ${ backgroundColorClass }`
                        : '' ),
                style: legacyStyle,
            };
        ...
        const legacyWrapperClasses = classnames(
            wrapperClasses,
            {
