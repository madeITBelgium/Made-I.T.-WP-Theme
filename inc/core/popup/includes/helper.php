...
function mp_is_popup_active($popup_id) {
    if (!get_field('popup_enabled', $popup_id)) return false;

    $now = current_time('timestamp');
    $start = get_field('popup_start', $popup_id);
    $end = get_field('popup_end', $popup_id);

    if ($start && strtotime($start) > $now) return false;
    if ($end && strtotime($end) < $now) return false;
...
function mp_get_popup_status($popup_id) {
    ...
    if (!$enabled) return ['status'=>'disabled','label'=>'<span style="color:#999">Uitgeschakeld</span>'];
    if ($start && strtotime($start) > $now) return ['status'=>'planned','label'=>'<span style="color:#0073aa">Gepland</span>'];
