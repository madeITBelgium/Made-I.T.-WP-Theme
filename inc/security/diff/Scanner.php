...
class Scanner {
    ...
    private static function gather_files(): array {
        ...
        if ( get_option( 'aswp_scan_plugins', true ) )  $dirs[] = wp_normalize_path( WP_PLUGIN_DIR ) . '/';
        if ( get_option( 'aswp_scan_themes', true ) )   $dirs[] = wp_normalize_path( get_theme_root() ) . '/';
        if ( get_option( 'aswp_scan_uploads', true ) )  $dirs[] = wp_upload_dir()['basedir'] . '/';
