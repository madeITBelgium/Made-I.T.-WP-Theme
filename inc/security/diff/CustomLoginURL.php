...
class CustomLoginURL
{
    ...
    {
        if (!get_option('aswp_custom_login_enabled', false)) {
            return;
        }
    ...
    {
        if (!get_option('aswp_custom_login_block_wpadmin', true)) {
            return;
        }
        ...
        if (!str_starts_with(ltrim($uri, '/'), 'wp-admin')) {
            return;
        }
        ...
        if (str_contains($uri, 'admin-ajax.php')) {
            return;
        }
...
<body><h1>Not Found</h1><p>The page you requested could not be found.</p></body></html>';
    ...
    {
        if (empty($_COOKIE['aswp_admin_grace'])) {
            return false;
        }
