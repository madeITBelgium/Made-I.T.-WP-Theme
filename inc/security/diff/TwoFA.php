...
class TwoFA {
    ...
    public static function init(): void {
        ...
        add_action( 'login_form_aswp2fa', [ __CLASS__, 'handle_2fa_form' ] );
        ...
        // AJAX
        add_action( 'wp_ajax_aswp_2fa_verify_totp',    [ __CLASS__, 'ajax_verify_totp'   ] );
        add_action( 'wp_ajax_aswp_2fa_generate_totp',   [ __CLASS__, 'ajax_generate_totp' ] );
        add_action( 'wp_ajax_aswp_2fa_send_email_otp',  [ __CLASS__, 'ajax_send_email_otp'] );
        add_action( 'wp_ajax_aswp_2fa_disable',         [ __CLASS__, 'ajax_disable_2fa'   ] );
        add_action( 'wp_ajax_aswp_2fa_enable_email',    [ __CLASS__, 'ajax_enable_email'  ] );
        add_action( 'wp_ajax_aswp_2fa_regen_codes',     [ __CLASS__, 'ajax_regen_codes'   ] );
        add_action( 'wp_ajax_nopriv_aswp_2fa_validate', [ __CLASS__, 'ajax_validate_2fa'  ] );
        add_action( 'wp_ajax_nopriv_aswp_2fa_resend',   [ __CLASS__, 'ajax_resend_otp'   ] );
    }
    ...
    public static function on_login( string $user_login, \WP_User $user ): void {
        ...
        if ( self::role_requires_2fa( $user ) && ! self::user_has_2fa( $user->ID ) ) {
            update_user_meta( $user->ID, 'aswp_2fa_enrollment_required', 1 );
            return;
        }
        ...
        $nonce = wp_create_nonce( 'aswp_2fa_challenge_' . $token );
        set_transient( 'aswp_2fa_pending_' . $token, [
            'user_id'  => $user->ID,
            'login'    => $user_login,
    ...
    public static function handle_2fa_form(): void {
        ...
        $data  = get_transient( 'aswp_2fa_pending_' . $token );
        ...
        if ( isset( $_SERVER['REQUEST_METHOD'] ) && $_SERVER['REQUEST_METHOD'] === 'POST' ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotValidated -- simple string comparison, no sanitization needed
            check_admin_referer( 'aswp_2fa_' . $token );
            $code = isset( $_POST['aswp_2fa_code'] ) ? sanitize_text_field( wp_unslash( $_POST['aswp_2fa_code'] ) ) : '';
            ...
            if ( $valid ) {
                delete_transient( 'aswp_2fa_pending_' . $token );
            ...
            if ( $remaining <= 0 || $attempts >= self::MAX_2FA_ATTEMPTS ) {
                ...
                set_transient( 'aswp_2fa_pending_' . $token, $data, $remaining );
            }
    ...
    private static function show_2fa_page( string $token, string $method, \WP_User $user, string $error = '' ): void {
        $form_nonce    = wp_create_nonce( 'aswp_2fa_' . $token );
        $data          = get_transient( 'aswp_2fa_pending_' . $token );
        ...
        wp_enqueue_style( 'aswp-2fa-page', ASWP_URL . 'includes/assets/css/twofa-page.css', [], ASWP_VERSION );

        if ( $method === 'email' ) {
            wp_enqueue_script( 'aswp-2fa-resend', ASWP_URL . 'includes/assets/js/twofa-resend.js', [], ASWP_VERSION, true );
            wp_localize_script( 'aswp-2fa-resend', 'aswp2faCfg', [
                'token'   => $token,
        ...
        echo '<div class="aswp-2fa-wrap">';
        echo '<h1>Two-Factor Authentication</h1>';
        if ( $error ) echo '<div class="aswp-2fa-error">' . esc_html( $error ) . '</div>';
        echo '<p>' . esc_html( $hint ) . '</p>';
        echo '<form method="post">';
        echo '<input type="hidden" name="_wpnonce" value="' . esc_attr( $form_nonce ) . '">';
        echo '<input type="text" name="aswp_2fa_code" class="aswp-code-input" maxlength="8" autocomplete="one-time-code" autofocus placeholder="000000">';
        echo '<button type="submit" class="aswp-2fa-submit">Verify</button>';
        if ( $method === 'email' ) {
            echo '<button type="button" class="aswp-2fa-resend" id="aswp-resend-btn">Resend code</button>';
        }
    ...
    public static function verify_totp( int $user_id, string $code ): bool {
        $secret = get_user_meta( $user_id, 'aswp_totp_secret', true );
        ...
        for ( $drift = -1; $drift <= 1; $drift++ ) {
            if ( self::compute_totp( $secret, $time + $drift ) === $code ) {
                ...
                $last = (int) get_user_meta( $user_id, 'aswp_totp_last_used', true );
                if ( $last === ( $time + $drift ) ) return false;
                update_user_meta( $user_id, 'aswp_totp_last_used', $time + $drift );
                return true;
            }
        }
        return false;
    ...
    public static function send_email_otp( int $user_id ): void {
        ...
        set_transient( 'aswp_email_otp_' . $user_id, password_hash( $code, PASSWORD_BCRYPT ), 600 );
    ...
    public static function verify_email_otp( int $user_id, string $code ): bool {
        $hash = get_transient( 'aswp_email_otp_' . $user_id );
        if ( ! $hash ) return false;
        if ( password_verify( trim( $code ), $hash ) ) {
            delete_transient( 'aswp_email_otp_' . $user_id );
            return true;
        }
        return false;
    }
    ...
    public static function verify_recovery_code( int $user_id, string $code ): bool {
        ...
        foreach ( $hashes as $i => $hash ) {
            if ( password_verify( $code, $hash ) ) {
                unset( $hashes[ $i ] );
                update_user_meta( $user_id, 'aswp_recovery_codes', array_values( $hashes ) );
                return true;
            }
        }
        return false;
    ...
    public static function enforce_enrollment(): void {
        ...
        if ( self::user_has_2fa( $user->ID ) || ! self::role_requires_2fa( $user ) ) {
            if ( get_user_meta( $user->ID, 'aswp_2fa_enrollment_required', true ) ) {
                delete_user_meta( $user->ID, 'aswp_2fa_enrollment_required' );
            }
            return;
        }
        ...
        wp_safe_redirect( add_query_arg( 'aswp_2fa_required', '1', admin_url( 'profile.php' ) ) . '#aswp-2fa-setup' );
        exit;
    }
    ...
    public static function enrollment_notice(): void {
        ...
        if ( ! isset( $_GET['aswp_2fa_required'] ) ) return; // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- display-only notice
        $user = wp_get_current_user();
    ...
    public static function user_has_2fa( int $user_id ): bool {
        return (bool) get_user_meta( $user_id, 'aswp_2fa_enabled', true );
    }

    public static function role_requires_2fa( \WP_User $user ): bool {
        if ( ! get_option( 'aswp_twofa_enabled', false ) ) return false;
        $raw      = get_option( 'aswp_twofa_required_roles', 'administrator' );
    ...
    public static function ajax_generate_totp(): void {
        ...
        update_user_meta( $user_id, 'aswp_totp_secret_pending', $secret );

        $uri = self::get_totp_uri( $user_id, $secret );

        // Generate QR code SVG server-side (no external API dependency).
        $qr_svg = '';
        if ( class_exists( 'FortressWP\\lib\\QRCode' ) || file_exists( ASWP_DIR . 'includes/lib/QRCode.php' ) ) {
            require_once ASWP_DIR . 'includes/lib/QRCode.php';
            $qr_svg = \FortressWP\lib\QRCode::svg( $uri, 200 );
        }
    ...
    public static function ajax_verify_totp(): void {
        ...
        $secret  = get_user_meta( $user_id, 'aswp_totp_secret_pending', true );
        ...
        if ( $valid ) {
            ...
            update_user_meta( $user_id, 'aswp_totp_secret', $secret );
            delete_user_meta( $user_id, 'aswp_totp_secret_pending' );
            update_user_meta( $user_id, 'aswp_2fa_enabled', true );
            update_user_meta( $user_id, 'aswp_2fa_method',  'totp' );
            update_user_meta( $user_id, 'aswp_totp_last_used', $time );
            delete_user_meta( $user_id, 'aswp_2fa_enrollment_required' );
    ...
    public static function ajax_validate_2fa(): void {
        ...
        if ( ! isset( $data['nonce'] ) || ! wp_verify_nonce( $nonce, 'aswp_2fa_challenge_' . $token ) ) {
    ...
    public static function enqueue_profile_script( string $hook ): void {
        ...
        wp_enqueue_script(
            'aswp-2fa-profile',
            ASWP_URL . 'admin/assets/js/2fa-profile.js',
            [ 'jquery' ],
            ASWP_VERSION,
            true
        );

        wp_localize_script( 'aswp-2fa-profile', 'aswp2fa', [
            'ajax_url' => admin_url( 'admin-ajax.php' ),
            'nonce'    => wp_create_nonce( 'aswp_nonce' ),
        ] );
    }
    ...
    public static function ajax_disable_2fa(): void {
        ...
        $user_id = get_current_user_id();
        delete_user_meta( $user_id, 'aswp_2fa_enabled' );
        delete_user_meta( $user_id, 'aswp_totp_secret' );
        delete_user_meta( $user_id, 'aswp_totp_secret_pending' );
        delete_user_meta( $user_id, 'aswp_2fa_method' );
        delete_user_meta( $user_id, 'aswp_recovery_codes' );
    ...
    public static function ajax_enable_email(): void {
        ...
        update_user_meta( $user_id, 'aswp_2fa_enabled', true );
        update_user_meta( $user_id, 'aswp_2fa_method',  'email' );
        delete_user_meta( $user_id, 'aswp_2fa_enrollment_required' );
        $codes = self::generate_recovery_codes( $user_id );
    ...
    public static function profile_section( \WP_User $user ): void {
        ...
        $method  = get_user_meta( $user->ID, 'aswp_2fa_method', true ) ?: 'totp';
        $codes   = count( (array) get_user_meta( $user->ID, 'aswp_recovery_codes', true ) );
        ?>
        ...
        <tr><th>Status</th><td>
            <?php if ( $enabled ): ?>
                ...
                &nbsp; <button type="button" class="button" id="aswp-disable-2fa">Disable 2FA</button>
        ...
        <tr><th>Enable 2FA</th><td>
            <select id="aswp-2fa-method-select" class="regular-text">
                <option value="totp">Authenticator App (TOTP)</option>
                <option value="email">Email OTP</option>
            </select>
            <br><br>
            <div id="aswp-totp-setup" style="display:none">
                <p><strong>Step 1:</strong> Scan this QR code with Google Authenticator, Authy, or any TOTP app.</p>
                <div id="aswp-qr-container" style="margin:12px 0;">
                ...
                <input type="text" id="aswp-totp-code" class="regular-text" placeholder="000000" maxlength="6" style="letter-spacing:.3em;font-size:1.2rem;width:140px">
                <button type="button" class="button button-primary" id="aswp-verify-totp">Verify &amp; Enable</button>
                <div id="aswp-totp-result"></div>
            </div>
            <div id="aswp-email-setup" style="display:none">
                <p>A one-time code will be emailed to <strong><?php echo esc_html( $user->user_email ); ?></strong> each time you log in.</p>
                <button type="button" class="button" id="aswp-send-test-otp">Send Test Code</button>
                <input type="text" id="aswp-email-otp-code" class="regular-text" placeholder="000000" maxlength="6" style="letter-spacing:.3em;font-size:1.2rem;width:140px;display:none">
                <button type="button" class="button button-primary" id="aswp-verify-email-otp" style="display:none">Verify &amp; Enable</button>
            </div>
            <button type="button" class="button" id="aswp-2fa-start-setup">Set Up 2FA</button>
        </td></tr>
        <?php endif; ?>
        <?php if ( $enabled ): ?>
        <tr><th>Recovery Codes</th><td>
            <button type="button" class="button" id="aswp-regen-codes">Regenerate Recovery Codes</button>
            <div id="aswp-recovery-codes-display" style="display:none;margin-top:12px"></div>
        </td></tr>
        <?php endif; ?>
        </table>
        <?php
    ...
    public static function save_profile( int $user_id ): void {
        ...
        if ( isset( $_POST['aswp_disable_2fa'] ) && current_user_can( 'edit_user', $user_id ) ) {
            delete_user_meta( $user_id, 'aswp_2fa_enabled' );
            delete_user_meta( $user_id, 'aswp_totp_secret' );
            delete_user_meta( $user_id, 'aswp_2fa_method' );
            delete_user_meta( $user_id, 'aswp_recovery_codes' );
