...
class RestApiPolicy {
    ...
    /** Option keys. */
    private const OPT_ENABLED  = 'aswp_rest_policies_enabled';
    private const OPT_POLICIES = 'aswp_rest_policies';

    /** Transient prefix for rate-limit counters. */
    private const RL_PREFIX = 'aswp_rest_rl_';
    ...
    public static function init(): void {
        ...
        add_action( 'wp_ajax_aswp_get_rest_policies',  [ __CLASS__, 'ajax_get_policies' ] );
        add_action( 'wp_ajax_aswp_save_rest_policies', [ __CLASS__, 'ajax_save_policies' ] );
        add_action( 'wp_ajax_aswp_get_rest_api_stats', [ __CLASS__, 'ajax_get_stats' ] );
    }
    ...
    public static function enforce_policies( $result, \WP_REST_Server $server, \WP_REST_Request $request ) {
        ...
        if ( class_exists( 'FortressWP\\Whitelist' ) && \FortressWP\Whitelist::is_allowed( $ip ) ) {
            return $result;
        }
        ...
        foreach ( $policies as $policy ) {
                ...
                if ( ! in_array( $method, $allowed_methods, true ) ) {
                    ...
                    return new \WP_Error(
                        ...
                        sprintf(
                            ...
                            __( 'Method %s is not allowed for this endpoint.', 'atlant-security' ),
                            $method
                        ),
                        [ 'status' => 405 ]
                    );
            ...
            if ( ! empty( $policy['auth_required'] ) && ! $logged_in ) {
                ...
                return new \WP_Error(
                    ...
                    __( 'Authentication is required to access this endpoint.', 'atlant-security' ),
                    [ 'status' => 401 ]
                );
            }
            ...
            if ( $rate_limit > 0 ) {
                ...
                if ( self::is_rate_limited( $rl_key, $rate_limit ) ) {
                    ...
                    return new \WP_Error(
                        ...
                        __( 'Rate limit exceeded. Please slow down.', 'atlant-security' ),
                        [ 'status' => 429 ]
                    );
                }
            }
    ...
    public static function get_policies(): array {
        $raw = get_option( self::OPT_POLICIES, '' );
    ...
    public static function ajax_get_policies(): void {
        ...
        wp_send_json_success( [
            'enabled'    => (bool) get_option( self::OPT_ENABLED, true ),
            'policies'   => self::get_policies(),
    ...
    public static function ajax_get_stats(): void {
        ...
        $total = (int) $wpdb->get_var( $wpdb->prepare(
            "SELECT COUNT(*) FROM {$wpdb->prefix}aswp_visitor_log
              WHERE url LIKE %s
                AND created_at >= %s",
            '%/wp-json/%',
            $cutoff
        ...
        $blocked = (int) $wpdb->get_var( $wpdb->prepare(
            "SELECT COUNT(*) FROM {$wpdb->prefix}aswp_events
        ...
        $top_routes = $wpdb->get_results( $wpdb->prepare(
            "SELECT
               ...
               FROM {$wpdb->prefix}aswp_visitor_log
              WHERE url LIKE %s
                AND created_at >= %s
              GROUP BY route
              ORDER BY hits DESC
    ...
    private static function log_block( array $policy, string $ip, string $route, string $method, string $reason ): void {
        ...
        if ( ! empty( $GLOBALS['aswp_current_log_id'] ) ) {
            // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching -- security data must not be served from cache
            $wpdb->update(
                $wpdb->prefix . 'aswp_visitor_log',
                [
                    'is_blocked'   => 1,
                    'block_reason' => 'REST Policy: ' . $policy_id,
                    'status_code'  => self::status_code_for_reason( $reason ),
                ],
                [ 'id' => $GLOBALS['aswp_current_log_id'] ],
                [ '%d', '%s', '%d' ],
                [ '%d' ]
            );
        }
