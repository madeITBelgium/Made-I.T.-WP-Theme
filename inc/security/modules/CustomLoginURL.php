...
class CustomLoginURL {
    ...
    public static function init(): void {
        if ( ! Settings::bool( 'madeit_security_custom_login_enabled', false ) ) return;
    ...
    public static function get_slug(): string {
        $slug = Settings::string( 'madeit_security_custom_login_slug', '' );
        if ( ! $slug ) {
            $slug = self::generate_slug();
            update_option( 'madeit_security_custom_login_slug', $slug );
        }
    ...
    public static function add_rewrite(): void {
        add_rewrite_rule(
            ...
            'index.php?madeit_security_login=1',
            'top'
        );
    }

    public static function add_query_var( array $vars ): array {
        $vars[] = 'madeit_security_login';
        return $vars;
    }

    public static function handle_request(): void {
        if ( ! get_query_var( 'madeit_security_login' ) ) return;

        // Generate a one-time token so block_real_login() will allow this redirect through.
        $token = wp_generate_password( 32, false );
        set_transient( 'madeit_security_login_token_' . wp_hash( $token ), 1, 120 );

        // Preserve standard wp-login.php query args across the redirect.
        $args = [ 'madeit_security_token' => $token ];
        ...
        // causing /custom-slug/?madeit_security_token=... → infinite loop.
    ...
    public static function block_real_login(): void {
        ...
        if ( ! empty( $_GET['madeit_security_token'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- one-time redirect token
            $token = sanitize_text_field( wp_unslash( $_GET['madeit_security_token'] ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            $key   = 'madeit_security_login_token_' . wp_hash( $token );
        ...
        // Safety net: recovery token bypass (e.g. ?madeit_security_recovery=<token>)
    ...
    public static function block_wpadmin_unauth(): void {
        if ( ! Settings::bool( 'madeit_security_custom_login_block_wpadmin', true ) ) return;
...
<body><h1>Not Found</h1><p>The page you requested could not be found.</p></body></html>';
    ...
    /**
     * Recovery token: append ?madeit_security_recovery=<token> to wp-login.php to bypass
    ...
    private static function has_valid_recovery_token(): bool {
        $token = isset( $_GET['madeit_security_recovery'] ) ? sanitize_text_field( wp_unslash( $_GET['madeit_security_recovery'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- recovery token bypass (no nonce available)
        if ( ! $token ) return false;
        $stored = Settings::string( 'madeit_security_recovery_token', '' );
    ...
    private static function has_admin_grace_cookie(): bool {
        if ( empty( $_COOKIE['madeit_security_admin_grace'] ) ) return false;

        $raw   = sanitize_text_field( wp_unslash( $_COOKIE['madeit_security_admin_grace'] ) );
        $parts = explode( '|', $raw );
    ...
    public static function set_admin_grace_cookie(): void {
        ...
        if ( PHP_VERSION_ID >= 70300 ) {
            setcookie( 'madeit_security_admin_grace', $value, [
                'expires'  => $expires,
                'path'     => COOKIEPATH ?: '/',
            ...
            setcookie( 'madeit_security_admin_grace', $value, $expires, COOKIEPATH ?: '/', COOKIE_DOMAIN ?: '', is_ssl(), true );
        }
    }
    ...
    private static function grace_cookie_key(): string {
        if ( function_exists( 'wp_salt' ) ) {
            return 'madeit-security-grace|' . wp_salt( 'auth' );
        }
        // Fallback — should never hit this path in a real WP runtime.
        return 'madeit-security-grace|' . ( defined( 'AUTH_SALT' ) ? AUTH_SALT : 'fallback-salt' );
    }
    ...
    public static function ensure_recovery_token(): string {
        $token = Settings::string( 'madeit_security_recovery_token', '' );
        if ( ! $token ) {
            $token = wp_generate_password( 32, false );
            update_option( 'madeit_security_recovery_token', $token, false );
        }
        return $token;
    }
    ...
    public static function recovery_url(): string {
        ...
        return site_url( 'wp-login.php?madeit_security_recovery=' . $token );
    }
    ...
    public static function ajax_save(): void {
        ...
        update_option( 'madeit_security_custom_login_enabled',      $enabled );
        update_option( 'madeit_security_custom_login_block_wpadmin', $block_admin );
        ...
        if ( $custom_slug && $custom_slug !== Settings::string( 'madeit_security_custom_login_slug', '' ) ) {
            ...
            update_option( 'madeit_security_custom_login_slug', $custom_slug );
        }
        ...
        update_option( 'madeit_security_flush_rewrite_pending', 1 );
