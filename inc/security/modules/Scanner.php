...
class Scanner {
    ...
    private static function get_signatures(): array {
        if ( ! empty( self::$signatures_cache ) ) return self::$signatures_cache;
    ...
    public static function init(): void {
        ...
        add_action( 'madeit_security_run_scan_batch',  [ __CLASS__, 'run_batch'   ] );
        add_action( 'madeit_security_run_db_scan',     [ __CLASS__, 'run_db_scan' ] );
        add_action( 'madeit_security_weekly_scan',     [ __CLASS__, 'run_weekly_scan' ] );

        add_action( 'wp_ajax_madeit_security_start_scan',      [ __CLASS__, 'ajax_start_scan'   ] );
        add_action( 'wp_ajax_madeit_security_scan_status',     [ __CLASS__, 'ajax_scan_status'  ] );
        add_action( 'wp_ajax_madeit_security_export_findings', [ __CLASS__, 'ajax_export_findings' ] );
        add_action( 'wp_ajax_madeit_security_get_scan_results',[ __CLASS__, 'ajax_scan_results' ] );
        add_action( 'wp_ajax_madeit_security_quarantine_file', [ __CLASS__, 'ajax_quarantine'   ] );
    ...
    public static function run_batch(): void {
        $state       = \MadeIT\Security\Settings::array( 'madeit_security_scan_state', [] );
        if ( ( $state['status'] ?? '' ) === 'complete' ) return;

        $batch_size  = \MadeIT\Security\Settings::int( 'madeit_security_scan_files_per_batch', 50 );
        $offset      = (int) ( $state['offset'] ?? 0 );
        $scan_id     = $state['scan_id'] ?? self::new_scan_id();

        // Build file list if not yet gathered (cron path)
        $all_files = \MadeIT\Security\Settings::array( 'madeit_security_scan_files_list', [] );
        if ( empty( $all_files ) && $offset === 0 ) {
            $all_files = self::gather_files();
            update_option( 'madeit_security_scan_files_list', $all_files, false );
            $state['total'] = count( $all_files );
            update_option( 'madeit_security_scan_state', $state, false );
        }
        ...
        if ( $new_offset >= count( $all_files ) ) {
            ...
            $state['completed'] = time();
            $state['status']    = 'complete';
            update_option( 'madeit_security_last_scan_completed', current_time( 'mysql' ) );
            ...
            // Continue automatically in cron context until complete.
            // AJAX status polling can also advance the scan, but this ensures
            // background scans (weekly cron) finish without manual polling.
            if ( ! wp_next_scheduled( 'madeit_security_run_scan_batch' ) ) {
                wp_schedule_single_event( time() + 30, 'madeit_security_run_scan_batch' );
            }
        }
    ...
    public static function run_db_scan(): void {
        ...
        $exclude_options = array( 'cron', '_site_transient_' . 'update_plugins', '_site_transient_' . 'update_themes' );
        ...
        $known_admins = \MadeIT\Security\Settings::array( 'madeit_security_known_admins', [] );
        ...
        if ( ! empty( $results ) ) {
            $existing = \MadeIT\Security\Settings::array( 'madeit_security_scan_db_results', [] );
            update_option( 'madeit_security_scan_db_results', array_merge( $existing, $results ), false );
            self::notify_threats( count( $results ), 'db_scan', 'database' );
        }

        update_option( 'madeit_security_last_db_scan', current_time( 'mysql' ) );
    }

    // ── Scan a single file ─────────────────────────────────────────────────────
    private static function scan_file( string $file, string $scan_id ): int {
        if ( ! is_readable( $file ) || is_dir( $file ) ) return 0;
        if ( filesize( $file ) > 5 * 1024 * 1024 ) return 0; // skip >5MB
        ...
        foreach ( $sigs_to_check as [ $sig_id, $name, $pattern, $severity ] ) {
            if ( preg_match( $pattern, $content, $matches ) ) {
                self::log_finding( $scan_id, $file, $sig_id, $name, $severity,
    ...
    private static function log_finding( string $scan_id, string $file, string $sig_id, string $name, string $severity, string $snippet ): void {
        $findings = \MadeIT\Security\Settings::array( 'madeit_security_scan_findings', [] );
        $findings[] = [
            'scan_id'   => $scan_id,
        ...
        update_option( 'madeit_security_scan_findings', $findings, false );
    ...
    private static function gather_files(): array {
        ...
        if ( \MadeIT\Security\Settings::bool( 'madeit_security_scan_core', true ) ) {
            $dirs[] = ABSPATH . 'wp-admin/';
            $dirs[] = ABSPATH . 'wp-includes/';
            // Root PHP files
            foreach ( glob( ABSPATH . '*.php' ) ?: [] as $f ) $files[] = $f;
        }
        if ( \MadeIT\Security\Settings::bool( 'madeit_security_scan_plugins', true ) )  $dirs[] = wp_normalize_path( WP_PLUGIN_DIR ) . '/';
        if ( \MadeIT\Security\Settings::bool( 'madeit_security_scan_themes', true ) )   $dirs[] = wp_normalize_path( get_theme_root() ) . '/';
        if ( \MadeIT\Security\Settings::bool( 'madeit_security_scan_uploads', true ) )  $dirs[] = wp_upload_dir()['basedir'] . '/';
        ...
        foreach ( $dirs as $dir ) {
            ...
            foreach ( $iter as $file ) {
                ...
                // Skip excluded
                $skip = false;
                foreach ( $exclude as $ex ) { if ( str_starts_with( $path, $ex ) ) { $skip = true; break; } }
                if ( $skip ) continue;
    ...
    public static function check_uploaded_file( int $attachment_id ): void {
        ...
        if ( $ext === 'php' ) {
            ...
            \MadeIT\Security\modules\Notifications::send_alert(
    ...
    public static function ajax_quarantine(): void {
        ...
        $quarantine_dir = wp_upload_dir()['basedir'] . '/madeit-security-quarantine/';
        ...
        if ( $wp_filesystem && $wp_filesystem->move( $full, $dest ) ) {
            ...
            foreach ( $findings as &$f ) {
                if ( $f['file'] === $file ) $f['status'] = 'quarantined';
            }
            update_option( 'madeit_security_scan_findings', $findings, false );
    ...
    public static function ajax_start_scan(): void {
        ...
        wp_send_json_success( [
            'message' => 'Scan started — ' . (int) ( $state['total'] ?? 0 ) . ' files to scan.',
            'state'   => $state,
        ] );
    }

    public static function run_weekly_scan(): void {
        if ( ! \MadeIT\Security\Settings::bool( 'madeit_security_scan_auto_weekly', false ) ) {
            return;
        }
    ...
    private static function start_scan(): void {
        $scan_id = self::new_scan_id();

        // Reset pending batch events from an older scan state.
        wp_clear_scheduled_hook( 'madeit_security_run_scan_batch' );

        // Reset findings
        update_option( 'madeit_security_scan_findings',   [], false );
        update_option( 'madeit_security_scan_db_results', [], false );

        // Gather files list upfront so we know the total immediately
        $files = self::gather_files();
        update_option( 'madeit_security_scan_files_list', $files, false );
        update_option( 'madeit_security_scan_state', [
            'scan_id' => $scan_id,
            'offset'  => 0,
            'total'   => count( $files ),
            'found'   => 0,
    ...
    public static function ajax_scan_results(): void {
        ...
        wp_send_json_success( [
            ...
            'last_scan'      => \MadeIT\Security\Settings::get( 'madeit_security_last_scan_completed', null ),
            'last_db_scan'   => \MadeIT\Security\Settings::get( 'madeit_security_last_db_scan', null ),
            'state'          => \MadeIT\Security\Settings::array( 'madeit_security_scan_state', [] ),
        ] );
    }
    ...
    private static function notify_threats( int $count, string $scan_id, string $area = 'filesystem' ): void {
        Notifications::send_alert(
            "⚠️ Security: $count threat(s) detected",
    ...
    public static function ajax_export_findings(): void {
        ...
        if ( ! current_user_can( 'manage_options' ) ) {
            wp_die( esc_html__( 'Unauthorized.', 'madeit' ), '', [ 'response' => 403 ] );
        }
        ...
        $filename = sprintf( 'Made I.T. Security-scan-%s-findings-%s-%s.csv', $kind, sanitize_file_name( $host ), $date_str );
        ...
        if ( false === $out ) {
            wp_die( esc_html__( 'Could not open output stream.', 'madeit' ) );
        }
        ...
        if ( 'db' === $kind ) {
            $rows = \MadeIT\Security\Settings::array( 'madeit_security_scan_db_results', [] );
            ...
            $rows = \MadeIT\Security\Settings::array( 'madeit_security_scan_findings', [] );
