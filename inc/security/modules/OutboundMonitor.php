...
class OutboundMonitor {
    ...
    /** Option keys. */
    private const OPT_ENABLED       = 'madeit_security_outbound_monitor_enabled';
    private const OPT_MODE          = 'madeit_security_outbound_mode';
    private const OPT_ALLOWLIST     = 'madeit_security_outbound_allowlist';
    private const OPT_BLOCK_PRIVATE = 'madeit_security_outbound_block_private';
    ...
    public static function init(): void {
        ...
        add_action( 'wp_ajax_madeit_security_get_outbound_log',        [ __CLASS__, 'ajax_get_outbound_log' ] );
        add_action( 'wp_ajax_madeit_security_save_outbound_allowlist', [ __CLASS__, 'ajax_save_outbound_allowlist' ] );
        add_action( 'wp_ajax_madeit_security_get_outbound_stats',      [ __CLASS__, 'ajax_get_outbound_stats' ] );
    }
    ...
    public static function intercept_request( $preempt, array $args, string $url ) {
        ...
        $mode   = \MadeIT\Security\Settings::string( self::OPT_MODE, 'log' );
        ...
        // very first request.
        if ( \MadeIT\Security\Settings::bool( self::OPT_BLOCK_PRIVATE, true ) && self::is_private_target( $url ) ) {
            ...
            return new \WP_Error(
                'madeit_security_outbound_blocked',
                __( 'Outbound request blocked by security policy: target resolves to a private IP address.', 'madeit' )
            );
        }
        ...
        if ( ! self::is_domain_allowed( $url ) ) {
            if ( 'enforce' === $mode ) {
                ...
                return new \WP_Error(
                    'madeit_security_outbound_blocked',
                    __( 'Outbound request blocked by security policy.', 'madeit' )
                );
            }
    ...
    public static function get_allowlist(): array {
        $custom = \MadeIT\Security\Settings::string( self::OPT_ALLOWLIST, '' );
    ...
    private static function get_log( int $limit = 500 ): array {
        ...
        $rows = $wpdb->get_results( $wpdb->prepare(
            ...
            FROM {$wpdb->prefix}madeit_security_outbound_log
            ORDER BY id DESC
            LIMIT %d",
            $limit
        ), ARRAY_A );
    ...
    public static function flush_log(): void {
        ...
        global $wpdb;
        $table = $wpdb->prefix . 'madeit_security_outbound_log';
    ...
    public static function ajax_get_outbound_log(): void {
        ...
        $total = (int) $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->prefix}madeit_security_outbound_log" );
    ...
    public static function ajax_get_outbound_stats(): void {
        ...
        wp_send_json_success( [
            ...
            'mode'           => \MadeIT\Security\Settings::string( self::OPT_MODE, 'log' ),
            'allowlist'      => self::get_allowlist(),
            'block_private'  => \MadeIT\Security\Settings::bool( self::OPT_BLOCK_PRIVATE, true ),
            'timestamp'      => current_time( 'mysql' ),
        ] );
    }
    ...
    /**
     * Record a security event in the madeit_security_events table.
     *
     * @param string $url         Target URL.
     * @param string $method      HTTP method.
