...
class RequestLogger {
    ...
    public static function log_current_request(): void {
        ...
        if ( defined( 'MADEIT_SECURITY_INTERNAL' ) && MADEIT_SECURITY_INTERNAL ) return;
        ...
        if ( is_admin() && function_exists( 'is_user_logged_in' ) && is_user_logged_in() ) return;

        if ( ! Settings::bool( 'madeit_security_log_enabled', true ) ) return;
    ...
    private static function analyze_ua( string $ua ): array {
        ...
        if ( str_contains( $ua_lower, 'headlesschrome' ) ) {
            $is_bot = true; $bot_score = 85; $ua_family = 'HeadlessChrome';
        } elseif ( str_contains( $ua_lower, 'phantomjs' ) ) {
            $is_bot = true; $bot_score = 90; $ua_family = 'PhantomJS';
        }
        ...
        if ( ! $is_bot ) {
            if ( str_contains( $ua_lower, 'chrome' ) )        $ua_family = 'Chrome';
            elseif ( str_contains( $ua_lower, 'firefox' ) )   $ua_family = 'Firefox';
            elseif ( str_contains( $ua_lower, 'safari' ) )    $ua_family = 'Safari';
            elseif ( str_contains( $ua_lower, 'edge' ) )      $ua_family = 'Edge';
            elseif ( str_contains( $ua_lower, 'opera' ) )     $ua_family = 'Opera';
            elseif ( str_contains( $ua_lower, 'msie' ) || str_contains( $ua_lower, 'trident' ) ) $ua_family = 'IE';

            // OS family
            if ( str_contains( $ua_lower, 'windows' ) )       $os_family = 'Windows';
            elseif ( str_contains( $ua_lower, 'macintosh' ) ) $os_family = 'macOS';
            elseif ( str_contains( $ua_lower, 'linux' ) )     $os_family = 'Linux';
            elseif ( str_contains( $ua_lower, 'android' ) )   $os_family = 'Android';
            elseif ( str_contains( $ua_lower, 'iphone' ) || str_contains( $ua_lower, 'ipad' ) ) $os_family = 'iOS';
        }
