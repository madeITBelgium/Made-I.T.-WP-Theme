...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const bgColorInput2 = document.getElementById('under_construction_background_color2');
    ...
    function updateTextColor() {
        ...
        const bgColor2 = bgColorInput2 ? bgColorInput2.value : '';

        // Bepaal contrastkleur op basis van de achtergrond waar de tekst effectief op staat.
        // In de templates staat de tekst doorgaans op background_color (kleur 1).
        const colorForContrast = (bgColor || '').trim() || (bgColor2 || '').trim();
        if (!colorForContrast) return;

        const contrastColor = getContrastYIQ(colorForContrast);
        textColorInput.value = contrastColor;
    }

    updateTextColor(); // initieel bij laden

    if (bgColorInput) bgColorInput.addEventListener('input', updateTextColor);
    if (bgColorInput2) bgColorInput2.addEventListener('input', updateTextColor);
});
